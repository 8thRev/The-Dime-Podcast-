...
Only cards whose **recording date is exactly 7 days from the run date** are processed. Cards with label **AI Research Completed** are skipped.
...
| `RESEARCH_EMAIL_CC` | No | CC on guest research emails (not the SEO report). Default `team@dimepodcast.com`; comma-separate for more |
| `TIMEZONE` | No | Default `UTC` (dates compared in UTC) |
| `DAYS_BEFORE_RECORDING` | No | How far ahead of the recording date to send research. Default `7` |
...
| Problem | Fix |
|--------|-----|
| No cards processed | Recording date must be exactly 7 days from run date; card in Pre; no “AI Research Completed” label yet. |
