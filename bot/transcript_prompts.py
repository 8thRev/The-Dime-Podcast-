...
def get_transcript_prompt(
    ...
    source_description: str = (
        "YouTube auto-generated captions — no speaker labels, no punctuation "
        "cleanup, and it may contain errors, especially around names, brands, "
        "and industry jargon"
    ),
) -> str:
...
Raw transcript ({source_description}):
---
{raw_transcript}
---
...
- "cleaned_transcript": the full transcript reformatted into readable paragraphs with proper punctuation. Add "Bryan Fields:" / "{guest_name or 'Guest'}:" speaker labels at each turn (plus any other named speaker's own label, if the raw transcript identifies additional real speakers beyond the host and guest), inferred from conversational context when the source has no true diarization, or carried straight through when it does. Bryan Fields is the podcast host. Fix obvious transcription errors in names, brands, and industry-specific terms using the episode/guest/company context above. This must be the full conversation cleaned up, not a summary or a shortened version.
