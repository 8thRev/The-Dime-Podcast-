...
- [x] **UTM tagging on every outbound link the site renders** (`app/lib/utm.ts`, applied in `app/lib/rss.ts`, `app/lib/newsletter.ts`, `app/lib/sponsor.ts`). Before this, exactly **one** URL on the site carried UTM params — the sponsor slot above. Everything else was unattributable for the reason already documented there: GA4 only auto-parses `utm_*`, and `rel="noopener noreferrer"` strips the Referer header, so an untagged outbound link lands in the destination's Direct bucket with nothing to attribute it to. That covered the 18 Newton links inside newsletter editions (7 of them pointing at the `/benchmark` conversion page), every link inside the Simplecast show-notes HTML, and every auto-detected guest company link. `docs/analytics-spec.md` Part 5.2 had flagged the resulting Direct-traffic pool as the highest-value outstanding item. Specifics worth keeping:
  - **The show-notes links are tagged at build time, not in Simplecast.** The episode page renders the feed's `<content:encoded>` HTML verbatim, and `lib/rss.ts` already post-processed that string twice (`unescapeMarkdownUnderscores`, `normalizeCohostName`), so one more pass tags every link on ~303 episode pages with **no Simplecast edits and no resync**. This does not reach Apple/Spotify listeners, who get Simplecast's copy directly — that half stays manual, and `docs/utm-conventions.md` carries the strings for it.
  - **A link that already has any `utm_` param is never touched.** This is what lets the automatic pass and hand-tagging in Simplecast coexist: hand-written always wins, nothing is ever double-tagged, and there is no migration or ordering problem. It also protects params that arrived with the data (e.g. share-sheet URLs carrying their own `utm_source`). Load-bearing in practice, not just in theory — the audit found **41 Newton links already hand-tagged in the live feed** with `utm_medium=pod&utm_campaign=simple_link`, all correctly left alone. (The earlier repo-wide grep missed these because show-notes HTML is fetched from Simplecast at build time and exists nowhere in the repo.)
  - **`NEVER_TAG_HOSTS`** — Apple, Spotify, YouTube, LinkedIn, X, Instagram, Facebook, TikTok, Simplecast, Kit, and our own domains. Those platforms strip or ignore `utm_*` and give publishers no campaign reporting on inbound params, so tagging only uglifies the URL in the one place (show notes) where readers see it raw. Tagging an *internal* link would be actively harmful: it starts a new GA4 session and overwrites the visitor's real acquisition source.
  - `utm_source` stays **`Thedime`** everywhere, matching the value the existing show-note reads already used, so reporting stays continuous. Distinct mediums keep the four surfaces separable: `referral`/`episode_sponsor` (templated slot), `show_notes`/`episode_show_notes` (in-notes links), `referral`/`guest_link` (company links), `newsletter`/`first_principles` (edition bodies). `utm_content` carries the episode or edition slug so a click resolves to one page.
  - Ordering constraint in `lib/rss.ts`: `extractCompanyFromShowNotes()` and the meta `description` are both derived from the **untagged** HTML. The tagging only rewrites href attribute values, so descriptions were unaffected either way — deriving from the raw copy makes that a structural guarantee rather than a coincidence.
  - `SPONSOR_URL` and `SPONSOR_ORG_ID` stay untagged: they feed JSON-LD, where a tracking param would break entity reconciliation against Newton's own Organization node.
  - **Dead 8th Rev links found and unlinked, as a direct result of the audit.** `eighthrevolution.com/the-dime/` was in the show-notes boilerplate of **all 304** episodes and `/monthly-report/` (on both `eighthrevolution.com` and `8threv.com`) on ~216 each — every episode page was rendering at least one dead outbound link, most three, all 404. Verified 2026-08-06 that both domains resolve to an AWS load balancer (`Server: awselb/2.0`), not this deployment: that origin 301s its own root to dimepodcast.com but 404s every deeper path, so the host rules in `next.config.js` never fire and cannot repair them (the note at `next.config.js:41` still holds). `repairDeadLinks()` in `app/lib/rss.ts` now fixes them at render time, splitting on what the surrounding copy is: `/the-dime/` is **unlinked with its text kept** (the "At Eighth Revolution (8th Rev), we provide services from capital to cannabinoid…" credit reads fine as prose), while `/monthly-report/` is **retargeted to `/newsletter`** — that anchor reads "Sign up for our playbook here:", so unlinking it would have left a CTA promising a link that isn't there on ~216 pages, and the First Principles newsletter is the monthly report's successor. The retarget is a site-relative path rather than an absolute dimepodcast.com URL: it makes this a true internal link with no cross-origin hop, keeps working on preview deployments, and stays out of `withUtm`'s reach (tagging an internal link would start a fresh GA4 session and overwrite the visitor's real acquisition source). The bare domain root is deliberately excluded — it still resolves. **Ordering constraint:** this must run *after* `extractCompanyFromShowNotes()`, which uses the `eighthrevolution.com` href as the end-boundary of the "Guest Links" section; stripping the anchors first would break guest-company detection across a large share of episodes. Confirmed pre-existing rot rather than a tagging regression by comparing bare vs parameterized status codes (identical 404s). Note this fixes **only the website** — the same links are live in Apple/Spotify show notes, the YouTube descriptions, and old LinkedIn posts, none reachable from this repo; a 301 on that load balancer is the one change that would fix all surfaces at once.
  - **Not** added to `verify-site.mjs` — a network request per third-party destination fails the "cheap" leg of the bar below. `scripts/audit-outbound-links.mjs` runs on demand instead, classifying every rendered outbound link as tagged / hand-tagged / skipped-by-design / **unexplained** and failing on the last bucket or on a campaign that doesn't belong to the page it renders on. It imports the real `isNeverTagHost` rather than keeping its own host list, so the audit can't drift into checking the tagging against itself.
...
- [x] **Fixes from an independent adversarial review of the above.** Measured before → after against served HTML:
  - **Truncated titles: 20 → 0.** `buildPageTitle()` gives the page-specific part a 41-character budget once `" — The Dime Podcast"` is appended. Episode titles fit; LinkedIn-style editorial headlines don't, and the cut landed mid-keyword — `THC Is Just an Ingredient — How Schedule…` amputated immediately before **"III"**, `There Is No Playbook for New York…` dropped **"Cannabis"**, and `Vertical Integration Isn't Strategy —…` ended on a dangling em dash. `og:title` and `twitter:title` carried the same mangled string, so every share card was broken too. Added a `metaTitle` frontmatter field used for the meta tags only, with `title` still driving the `<h1>` and listings, and wrote a ≤41-char value for all 31 that leads with the category term rather than the curiosity hook (only 12 of 31 headlines contained *any* of cannabis/THC/CBD/hemp).
  - **Truncated descriptions: 12 → 0.** Rewritten by hand rather than left for `truncateDescription` to amputate. Two also closed real keyword gaps found by the review: **"cannabis banking" appeared zero times across all 31 files** despite the payments edition being the best-fit page for that query, and the CBD edition never said **"Schedule III"** even though its episode is literally *"Schedule III Changes Everything"*.
  - **Editions with zero inbound links from other editions: 26 → 0** (range 0–30 → 1–9). "More First Principles" took the 4 newest editions, so those 4 absorbed every internal link in the set and the other 26 got none. Now `getRelatedEditions()` sorts by shared topics, tie-broken by *ring distance in publication order* — the ring is what guarantees no orphan, since an edition sharing no topic with anyone is still its neighbours' nearest tie. Plain recency as the tie-break still left 2 of 31 unlinked.
  - **A title fragment was being published as a person's name.** The edition page took the guest from `episode.guest` (RSS-derived) instead of the frontmatter, so the Lyft edition rendered a link labelled *"Playbook for Cannabis: Building Trust Through Tech ft. Ashwin Raj"* — as visible body copy and as anchor text on a brand-new indexable page — while the correct name sat unused in frontmatter. Now prefers frontmatter, and only renders a link when a guest entity page actually exists for that name (otherwise plain text, so a correct-but-pageless name like Ashwin Raj can't publish a 404). The link appears on its own once `extractGuest()` is fixed.
  - **Duplicate `og:type` on every page of the site: 31 → 0 (sitewide).** `_document.js` declared an `og:type` default while `SeoHead` emits a per-page one, and next/head isn't deduped against `next/document`'s Head — the exact trap the file's own comment warns about for `<meta name="description">`. Pre-existing, affected every page, not just these.
  - **The homepage claim was false.** `/` and `/newsletter` both promised "NO SPONSORSHIP CONTENT. NO PARTNER PROMOTIONS." while 18 of the 31 editions carry Newton reads. Reworded to "NO THIRD-PARTY SPONSORSHIPS. NO PAID PLACEMENTS." — accurate, since Newton is a sister company rather than a paid placement. The homepage newsletter band also now links to the archive, which it didn't.
  - Also: `h1 → h3` heading skip fixed in the Schedule III edition, and `getAllEditions()` memoized (it re-read and re-parsed all 31 files on every call — once per episode page, guest page and topic hub).
  - **Correction to the note above**: the `extractGuest()` damage is larger than first measured — **29** slugs carry role/company junk (not 13), **5** aren't people at all (`las-vegas-newest-attraction`, `helping-cannabis-start-ups-light-up`), **Kim Rivers is split** across `kim-rivers-part-1`/`part-2` with no canonical, and **74 of 303** episodes produce no guest entity. Deferred to its own branch — it's a 222-page entity refactor, not a content change.
  - **Correction on LinkedIn gating**: the articles are *not* auth-gated. A plain fetch returns HTTP 200 with the full body; they're blocked by robots.txt only. The GEO argument is unaffected (LinkedIn disallows GPTBot, ClaudeBot, PerplexityBot, CCBot, Google-Extended *and* `*`), but **Googlebot is explicitly allowed**, so the Google-side duplicate-content risk is realer than first stated. Since the on-site copies are near-verbatim, the cross-links and schema are chrome rather than substantive differentiation — the answer-first summary + FAQ blocks below are what would actually make the on-site version a different document.
...
- [x] **Part 6 of `docs/analytics-spec.md` — all eight SEO/GEO gaps** (branch `seo-geo-part6`). Per-gap departures are tabulated in that doc's implementation-status section; what's worth keeping here is what the exercise found:
  - **`llms.txt` split into `/llms.txt` (17.9KB) + `/llms-full.txt` (699KB)**, shared builder in `app/lib/llms.js` so the two can't drift. The single file had reached 714KB, which is not a large index but a silently truncated one. Both unbounded lists it draws from are now capped — 25 recent episodes, 12 recent essays, descriptions cut to 200 chars (topics stay uncapped: a fixed taxonomy of 20 is bounded by the content model, and truncating it would make the hub list wrong rather than shorter). **`verify-site.mjs` check 14 asserts the 20KB budget**, and it earned its keep immediately: the first implementation came in at 39.6KB, because the AI episode summaries average **773 characters** each and 25 of those lines are 23KB on their own.
  - **Gap 2 ("llms.txt is stale, move generation into the build") is wrong and was not implemented.** The file has always been server-rendered per request from live RSS; the 304-vs-306 discrepancy was a stale CDN copy under `s-maxage=3600`. Doing what the spec asks would have made it *staler* — regenerating on deploy instead of hourly. The count now reads `episodes.length` so it matches the list beneath it.
  - **9 AI crawlers added to robots.txt**, including `OAI-SearchBot`, which is what backs ChatGPT *search* citations and is a different agent from `GPTBot`. The list is now split into retrieval vs training groups in `next-sitemap.config.js`: allowing only `GPTBot` was opting into training and out of the thing that actually sends referrals.
  - **287 `<video:video>` sitemap entries.** `<video:description>` is capped at 2048 characters and **235 of 287 catalogue descriptions exceed it** (max 4,993) because they carry the full YouTube description including chapters and links. An over-length field makes Google reject the whole entry, so they're truncated at the last whole word. This also gave video URLs a real `lastmod` from the committed catalogue — closing the "video lastmod is the build timestamp" item in the Hub layer below **without** the YouTube Data API call that item assumed it needed.
  - **`Clip` key-moment schema on 263 of 287 video pages**, parsed from the chapter list in each upload's own description (`app/lib/chapters.ts`) rather than from `raw_captions_srt` — captions carry timings but no section names, so titles would have had to be invented. 31 descriptions list chapters running past the upload's own duration (the list was written against a longer cut); the overrunning tail is dropped rather than discarding an otherwise good list. Chapters render visibly too, since structured data is supposed to describe what's on the page. Two things an independent review caught before merge: **`Clip.url` has to share the video page's own path** (a youtube.com watch URL, the natural choice because it seeks natively, makes the entire `hasPart` array ineligible), so clips point at `/videos/<slug>?t=N` and the page now honours `?t=` by passing `start=` to the embed; and **61 of 287 descriptions write chapter *ranges*** (`00:00 - 03:04 Title`), whose end timestamp was being glued to the front of 658 clip names — offsets were right, labels were not. Both fixed and re-verified across all 4,070 clips.
  - **Transcripts now render on video pages** — but the spec's "You already have every transcript" is false. **112 of 287** video pages get one. The ceiling is transcript coverage (116 of 304 episodes), i.e. the backfill in "MVP — now" below, not this work. FAQPage schema stays exclusive to the episode page: two URLs claiming the same Q&A pairs is the duplication Google actually collapses, unlike a shared transcript on two self-canonical, mutually-linked pages answering watch vs read intent.
  - **`interactionStatistic` is wired but inert until the next `video-catalog.yml` run.** `content/videos.json` only stores the display string (`"1.2K views"`), and parsing that back would publish 1,249 as a confident 1,200. `viewCountRaw` was added to the catalogue generator; the schema and sitemap both omit the node until it appears, so nothing is wrong in the meantime — it just isn't there yet. No local YouTube API key, so it could not be backfilled in this branch.
  - **Two site feeds**: `/rss.xml` (50 newest episodes) and `/newsletter/rss.xml` (all 32 essays), with `/feed` and `/feed.xml` 301'ing to the former. Neither carries an `<enclosure>` and `verify-site.mjs` asserts they never will — a site feed that looks like the podcast feed is how an aggregator ends up republishing the show from the wrong URL. The Simplecast feed keeps the unqualified "RSS" title in `_document.js`.
  - Verified against a running production build: `npm run build` green, `verify-site.mjs` **PASS** (19/19 routes covered, 902 sitemap URLs, 0 duplicates, 0 non-200, 80-page crawl clean), both feeds parse with `rss-parser`, and every new JSON-LD block parses with the expected keys.
- [x] **Automatic indexing for new First Principles editions** (`.github/workflows/index-new-pages.yml`, `bot/index_new_pages.py`). After each successful Vercel production deploy, the workflow finds any newsletter markdown that was added or changed. It checks the edition, its related episode page and `/newsletter` all serve 200, then submits them to **IndexNow** (Bing, Yandex and the other participating engines; Bing is what ChatGPT search and Copilot draw on). The key file is `app/public/c75b5bdcf73876e49ba9bf0f8a001402.txt`. It also resubmits `sitemap.xml` to Search Console. Things worth knowing:
  - **Google has no automated "Request indexing" for articles.** The Indexing API is only allowed for JobPosting and BroadcastEvent pages. Resubmitting the sitemap, plus internal links from `/newsletter`, the episode page and the guest page, is the automated limit. For a page that matters, Search Console's "Request indexing" button is still worth a click.
  - **The Google leg is best-effort.** Resubmitting a sitemap needs the `webmasters` scope and Full or Owner permission on the property. The SEO report's service account may only have read access, so a failure shows as a warning on the run and doesn't fail the job.
  - It triggers on `deployment_status`, not `push`, to keep video-catalog's "no workflow triggers on push" loop-safety invariant. Daily catalog deploys trigger it too, but they have no edition changes, so it exits in seconds. `workflow_dispatch` accepts paths for a manual resubmit.

- [x] **Weekly Report v2, Phase 1: AI and search visibility measured every Monday** (`bot/weekly_report.py`, spec `docs/report-v2-spec.md`). Writes `data/weekly/YYYY-MM-DD.json` and `data/reach.csv` from the existing SEO report workflow. The GEO work above now has a scoreboard:
  - **AI citation rate**: a fixed panel of questions (`data/ai_prompts.csv`) asked to Claude with web search each week, recording whether the answer cites dimepodcast.com or The Dime's YouTube videos, and which competitor domains get cited instead. AI assistants do not expose user prompts, so this is the direct measure.
  - **AI referrals** to the site (GA4 "AI Assistant" channel plus known assistant domains, with landing pages) and to YouTube videos (external referrer detail). Replaces the one number `ai_referrer_sessions()` gave the old report.
  - **Search**: YouTube search views (49 percent of episode video views in the first pull), branded vs non branded Google queries, question style queries as the AI Mode proxy, and YouTube thumbnail impressions and CTR from a Reporting API job.
  - First pull found GA4 total sessions are about 90 percent bot traffic (Direct, landing on "/", under 3 seconds of engagement), so only Organic Search and AI Assistant sessions are trusted as site numbers.

- [ ] **Agent readiness (queued ahead of Report v2 Phases 2 and 3, order in `docs/report-v2-spec.md`).** Crawlers are served well (SSR pages, `llms.txt`, AI crawlers allowed, IndexNow). Agents acting for people are not: the guest and sponsor forms silently discard any submission that fills the hidden "Website" honeypot, which agents and autofill do; `llms-full.txt` is 861 KB and growing; nothing records which pages agents fetch. In order: fix the honeypots (shipped, PR #62), log agent visits into the weekly snapshot (**shipped**: `app/src/middleware.js` counts fetches by 24 known agents into Upstash Redis, read by `bot/agent_visits_client.py`; the retrieval class fetch count is a reach column), `.md` page variants and per topic `llms.txt` (shipped, see below), agent actions and sponsor facts in `llms.txt`, then an MCP server over the catalogue.

- [x] **Quick wins from the external growth audit (2026-09-16, branch `site-audit-quick-wins`).** The audit (v1.1, "Prepared by Alfred") was partly wrong about the site: it reported no Apple/Spotify path on the homepage (the hero has icon-only buttons) and no guest-pitch form (`/guests` has one), and it recommended Podcast schema that had already shipped. What it got right:
  - **"Weekly" was false.** The latest episode was Aug 4, 2026, and the four before it ran 2–3 weeks apart. Cadence claims were softened on home, about, sponsorship, `/videos` and the `/newsletter` meta description rather than removed outright ("Since 2020 · On Air", "Conversations with…"). **Still open:** the Kit form's own success message ("Look out for new releases every week!") lives in the verbatim embed in `ConvertKitEmbed.js` and has to be changed in Kit first, then re-copied.
  - **Rating is now fetched, not hand-typed.** `lib/ratings.ts` reads `content/ratings.json`, which `scripts/update-apple-ratings.mjs` refreshes daily (`.github/workflows/apple-ratings.yml`) by summing every Apple storefront's show page. Apple ratings are per-country, and the figure had drifted: the hard-coded 115 didn't match Apple's 114 (US 108, CA 3, AU 1, DE 1, PH 1). The script refuses to publish if the US page can't be read or if the count drops by more than 5, keeps a storefront's last figures when a fetch fails, and only commits on change.
  - **Newsletter signup on every episode page**, after the show notes, tracked as `signup_location: episode_inline` (already a registered value) with `episode_slug`.
  - **The site never linked its own social accounts**, which is why the audit marked LinkedIn "not found." Instagram and the LinkedIn company page (`/company/the-dime-podcast`, org 28969050) were added to the footer and to the Organization `sameAs`. **Facebook is still missing**: the page URL couldn't be confirmed from outside a login wall.
  - Nav label "Guests" → **"Be a Guest"** (the bare label read as a list of past guests), plus a soft "Already a listener?" Apple-rating note on `/guests`. The longer label crowded the nav at 481–720px, so the hamburger breakpoint moved from 480 to 720.

- [x] **Markdown page variants and per topic `llms.txt` (agent readiness, step 3).** Every episode, guest, First Principles edition and topic hub now has a plain text twin at its own URL plus `.md` (`/episodes/<slug>.md`, `/guests/<slug>.md`, `/newsletter/<slug>.md`, `/topics/<topic>.md`), served as `text/markdown` by `app/src/pages/api/markdown/[kind]/[slug].js` behind a rewrite in `next.config.js`, and built in `app/lib/markdown.js` from the same loaders the HTML pages use so the two cannot drift. Each carries title, one line summary, date, guest, canonical URL, audio and video links; episodes add show notes, takeaways, FAQ and the transcript under an AI note, editions carry the essay verbatim with a byline and no disclosure. Each HTML page links its twin with `<link rel="alternate" type="text/markdown">` (via `SeoHead`), and a request for the HTML URL with `Accept: text/markdown` is 302'd to the `.md` (a routing-layer redirect, so the CDN's cached HTML is never touched). `/topics/<slug>/llms.txt` is one topic's index (episodes with one line summaries, essays, and every entry's `.md` URL), built by `buildTopicLlms` in `app/lib/llms.js` from the same line builders as `/llms.txt`; the biggest hubs come to about 43 KB against 861 KB for the full catalogue. `/llms.txt` and `robots.txt` state the `.md` rule and the per topic index once each, as a rule rather than per line, which keeps the index at 18.2 KB of its 20 KB budget. `verify-site.mjs` check 14 now fetches one `.md` of each kind, one topic index, and the `Accept` redirect. **Still open** from the agent readiness item: agent visit logging, agent actions and sponsor facts in `llms.txt`, the MCP server.

- [x] **The Answers column (`/answers`), written by the Isaac Burner bot.** A third content type at `app/content/answers/*.md`, loaded by `app/lib/answers.ts` and rendered by `app/src/pages/answers/[slug].js`. Each post answers one question, takes a position, and cites the episodes it drew from. Written by `bot/isaac_blogger.py` twice a week and **opened as a pull request**, never pushed to main.

  - **Why this is the highest-leverage GEO item that was left.** `FAQPage` was the site's strongest answer-engine asset and it existed on exactly one page type, gated behind transcript coverage (143 of 304 episodes). The column produces `FAQPage` pages that are not gated on anything except a question worth answering, and unlike an episode page it targets a question directly rather than a conversation that happens to contain one. Every post emits `FAQPage` (the headline question plus 2 to 4 related pairs, rendered visibly on the page, because schema whose answers are not visible is a structured-data violation) alongside `Article` via the new `createAnswerSchema()`.

  - **The disclosure is the design, not decoration.** The site's whole credibility split is that AI text carries `<AIDisclosure>` and human text does not, and `llms.txt` labels First Principles "Not AI-generated" directly above everything else. A named byline on AI-written opinion would quietly break that claim for every section below it. So: the byline is "Isaac Burner, AI analyst for The Dime" and that qualifier travels with it on the page, in the `.md` twin and in `llms.txt`; `createAnswerSchema()` sets `author` to the Organization rather than fabricating a `Person`, puts the persona in `creditText`, and carries a `disclaimer`. The Answers section in `llms.txt` is titled "Answers (AI written column)" and sits below First Principles.

  - **Grounding is enforced in code, not asked for in the prompt.** `rank_sources()` scores the catalogue against the question by term overlap and requires at least two distinct question terms and 40% of them before a question is used at all, so a query the catalogue cannot support produces no post rather than a generic one. `validate_post()` then rejects off-taxonomy topics, a cited slug that was not in the source material, an episode listed in frontmatter but never linked in the body, a body outside 350 to 700 words, and a title that is not a question. This is what keeps the column on the right side of Google's scaled content abuse policy: the cadence is two a week, every post is grounded in proprietary transcripts, and a human merges it.

  - **Question source.** Search Console queries from the last 90 days that are question shaped, get at least 2 impressions, and sit worse than average position 5. That is demand that exists and is currently landing on somebody else's page. Search Console is optional; a missing service account or an API outage falls through to unanswered FAQ pairs from transcripts of episodes the column has not cited yet, so a run degrades rather than fails.

  - **`/llms.txt` gets a pointer, not a listing.** The index was at 18.2 KB of its 20 KB budget and this column grows every few days, so listing the questions there would have passed on the day it shipped and failed the build within a month. The questions are listed in full at `/llms-full.txt` and in each topic's own index. The index is now 18.5 KB.

  - Wired into the sitemap (`/answers` at 0.8, posts at 0.7 with a real frontmatter `lastmod`), `robots.txt`, the `.md` rewrite and `Accept` negotiation, the per topic `llms.txt`, and `verify-site.mjs` checks 14 and 15. **Open**: `/answers` is linked sitewide from the footer only, not the header nav (seven items already), and topic hub *pages* list a topic's editions but not its answers, though the topic `llms.txt` does. Both are worth revisiting once the column has a body of posts. A Kit signup block on the post page is also deliberately deferred: `signup_location` is an enumerated GA4 dimension and a sixth value has to be registered first.
...
- [ ] **Fix `extractGuest()` in `app/lib/rss.ts`** (own branch). Measured over all 303 live feed items: 29 of 222 guest slugs carry role/company/title junk (`rena-sherbill-senior-editor-at-seeking-alpha-and-host-of-the-cannabis-investing-podcast`), 5 are not people at all (`las-vegas-newest-attraction`, `connecting-professional-cannabis-leaders`, `helping-cannabis-start-ups-light-up`, plus two title fragments), Kim Rivers is fragmented across `kim-rivers-part-1`/`part-2` with no canonical `kim-rivers`, and 74 episodes yield no guest entity at all. These are indexed URLs emitting `Person` JSON-LD with a non-person `name` — a structured-data correctness problem, not cosmetics. Bounded fix: tighten the extraction and add a hand-curated override map alongside the existing `GUEST_COMPANY_MAP`. Unblocks two newsletter guest cross-links for free (Ashwin Raj, Jonathan Black), which already carry the correct names in frontmatter.
- [ ] **Answer-first blocks on the newsletter editions** — the highest-value remaining GEO item, and the one thing that would make the on-site copies substantively different documents from the LinkedIn originals rather than better-wrapped duplicates. 30 of 31 bodies currently have no headings at all: an unbroken run of `<p>` with no passage boundaries for an extraction model to anchor on (which also leaves the `.prose-body h2/h3` CSS as dead code). In priority order: (a) a 2-3 sentence answer-first standfirst above the body, from a new `summary` frontmatter field distinct from the meta `description`; (b) 2-4 `##` section headings per edition; (c) 2-3 hand-written Q&A pairs per edition emitted as `FAQPage` JSON-LD through the existing `createFAQSchema()`. (c) is the single highest-leverage addition and reuses code that already ships — `FAQPage` is the type most often lifted into AI Overviews, and it's the one thing episode pages have that these don't. Needs Bryan's voice, so it's authoring work, not a code change.
- [ ] **Per-edition `og:image`** — every edition serves the generic `og-default.jpg`. For an audio episode page that's tolerable; for a text article the card image *is* the click driver on the channels this content travels on. These 31 are also the easiest case of the site-wide `og:image` gap: no rights problem, no external source, just a templated title card.
- [ ] Schema additions for `createArticleSchema()`: `dateModified`, `author.url` → `/about` (the site already emits `Person` schema for Bryan there, so the two should resolve to one entity instead of an unresolvable bare `author`), `inLanguage`, `mentions` for companies named in the body, and `name` + a matching `@id` on the `about` node so it resolves to the episode page's own `PodcastEpisode` node — the same `@id` trick the sponsor schema already uses. `BlogPosting` over `Article` isn't worth the churn; `speakable` is news-publisher-only, skip it.
- [ ] Link in-body person and company mentions to existing `/guests/[slug]` pages — the largest untapped internal-linking surface in the set (e.g. the Dr. Greenhouse edition names David Fetner, Hirsh Jain and Richie Proud in plain text, all three of whom have guest pages).
- [ ] `app/scripts/import-linkedin-editions.mjs` is dead code — it targets the personal LinkedIn data export, which by construction never contains company-page newsletters. Either delete it with its `turndown`/`node-html-parser` devDependencies, or rewrite its header so the next person doesn't reach for it.
...
- [x] ~~Sitemap `lastmod` for video pages still uses the build timestamp~~ — done as part of the Part 6 video-sitemap work (see Shipped). Neither option this item proposed was needed: `content/videos.json` is already committed and carries `publishedAt`, so `next-sitemap.config.js` reads it straight off disk for both the `lastmod` and the `<video:video>` entry. No API call, no build-time cache file.
