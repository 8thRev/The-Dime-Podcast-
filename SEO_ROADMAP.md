...
- [x] **Part 6 of `docs/analytics-spec.md` — all eight SEO/GEO gaps** (branch `seo-geo-part6`). Per-gap departures are tabulated in that doc's implementation-status section; what's worth keeping here is what the exercise found:
  - **`llms.txt` split into `/llms.txt` (17.9KB) + `/llms-full.txt` (699KB)**, shared builder in `app/lib/llms.js` so the two can't drift. The single file had reached 714KB, which is not a large index but a silently truncated one. Everything the index emits is now capped, because every list it draws from grows forever — 25 recent episodes, 12 recent essays, descriptions cut to 200 chars. **`verify-site.mjs` check 14 asserts the 20KB budget**, and it earned its keep immediately: the first implementation came in at 39.6KB, because the AI episode summaries average **947 characters** each and 25 of them are 23KB on their own.
