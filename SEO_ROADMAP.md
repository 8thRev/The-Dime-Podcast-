...
- [x] IndexNow submission (`app/scripts/submit-indexnow.mjs`, `.github/workflows/indexnow.yml`) — pings Bing/Yandex/Seznam/Naver the moment new episode content lands, instead of waiting on their recrawl schedule. Google does not participate in IndexNow, so the payoff is specifically Bing, which is what backs ChatGPT search and Copilot. The key is public by design (ownership is proven by serving it at `/<key>.txt`), so it lives in `app/public/` rather than in a secret and needs no CI configuration. Submits only URLs derived from the git diff — the protocol asks senders not to resubmit unchanged URLs and engines throttle those that do — with `--all` reserved for a one-time full-sitemap seed, a 200-URL cap on automatic runs, and a pre-submit HTTP check so a URL that 404s (e.g. an episode page whose deploy has not finished) is dropped rather than submitted. The ping is wired into `transcript-pipeline.yml` directly rather than triggered by `indexnow.yml`'s push event, because that pipeline's auto-commits carry `[skip ci]` and GitHub skips push-triggered workflows for those commits. (Vercel does not honour `[skip ci]` — only Actions does — so those commits still deploy.)
