...
- [ ] **Agent readiness (queued ahead of Report v2 Phases 2 and 3, order in `docs/report-v2-spec.md`).** Crawlers are served well (SSR pages, `llms.txt`, AI crawlers allowed, IndexNow). Agents acting for people are not: the guest and sponsor forms silently discard any submission that fills the hidden "Website" honeypot, which agents and autofill do; there is no plain text version of any page (`/episodes/<slug>.md` is 404, `Accept: text/markdown` returns HTML); `llms-full.txt` is 861 KB and growing; nothing records which pages agents fetch. In order: fix the honeypots, log agent visits into the weekly snapshot (**shipped**: `app/src/middleware.js` counts fetches by 24 known agents into Upstash Redis, read by `bot/agent_visits_client.py`; the retrieval class fetch count is a reach column), `.md` page variants and per topic `llms.txt`, agent actions and sponsor facts in `llms.txt`, then an MCP server over the catalogue.
